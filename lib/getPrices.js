...
function aequitas(rows) {
  ...
  // Aequitas is returning blank for the price for any request after the first.
  // I assume this is to prevent scraping. Select one price at random to fetch.
  // There are only two symbols we're interested in, anyway.
  const want = Math.floor(Math.random() * rows.length);

  return Promise.each(
    rows,
    (row, index) => {
      if (index === want) {
        return driver.get(`${baseUri}/${row.symbol}`)
      ...
      row.price = ''; // eslint-disable-line no-param-reassign
