      ...
      await driver
        ...
        .then(() =>
          driver.executeScript(
            `
              return [
                document.querySelector('#security-summary-table tr td:nth-child(2)').innerText,
                document.querySelector('#securityDetails > div:nth-child(2) > span:nth-child(3) > strong').innerText
              ]
            `
          )
        )
        .then(([price, trading]) => {
          if (trading.trim() === 'REGULAR TRADING') {
            r.price = price.replace(/\$/, '');
          } else {
            r.err = `${
              r.symbol
            } market not open for regular trading [${trading.trim()}]`;
          }
        })
        .catch(e => {
