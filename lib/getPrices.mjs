...
async function aequitas(rows) {
  ...
  const baseUri = 'https://www.cboe.com/markets/ca/equities/securities';
    ...
    .reduce(async (acc, row, idx) => {
      ...
      await page
        ...
        .then(() => {
          return Promise.all([
            page
              .locator('h2.tw-text-ink-brand-primary:nth-child(2)')
              .evaluate((node) => node.innerText),
            page
              .locator('div.tw-items-center:nth-child(2) > span:nth-child(2)')
              .evaluate((node) => node.innerText),
          ]);
        })
        .then(([price, trading]) => {
...
async function alphavantageFetch(row) {
  ...
  return fetch(baseUri + row.symbol)
    ...
    .then((json) => {
      ...
      console.log(
        `${logtime} got date=${r.date}, price=${r.price} fetching current price for ${row.symbol}`
      );
    })
    .catch((err) => {
